...
//Añadir elementos de las vistas 
	@FXML
	private ChoiceBox<Pokemon> choiceHembra;
	@FXML
	private ChoiceBox<Pokemon> choiceMacho;
	@FXML
	private ImageView imgCria;
	@FXML
	private ImageView imgHuevoMasc;
	@FXML
	private ImageView imgPokeF;
	@FXML
...
//metodo con el que iniciamos la crianza del pokemon el cual tendremos que coger un macho y una hembra del mismo tipo primario
	...
	public void inicializarCrianza(List<Pokemon> equipoYCaja) {
		List<Pokemon> machos = equipoYCaja.stream()
				.filter(p -> "M".equalsIgnoreCase(p.getSexo()) && p.getFertilidad() > 0).collect(Collectors.toList());

		List<Pokemon> hembras = equipoYCaja.stream()
				.filter(p -> "F".equalsIgnoreCase(p.getSexo()) && p.getFertilidad() > 0).collect(Collectors.toList());
...
//Metodo que nos dice si es compatible o no nuestro pokemon
	...
	void fusionarHuevo(MouseEvent event) {
		...
		nuevoHijo = new Pokemon(0, padreMacho.getIdEntrenador(), padreMacho.getNumPokedex(), nombre, tipo1, tipo2,
				vitalidad, ataque, defensa, atEspecial, defEspecial, velocidad, 1, 5, sexoHijo, "Sano", 0,
				parentVisual.getImgFrontal(), parentVisual.getImgTrasera(), parentVisual.getSonido(),
		...
		JOptionPane.showMessageDialog(null, "¡Ha nacido un " + nuevoHijo.getNombre() + "!", " Crianza completada",
