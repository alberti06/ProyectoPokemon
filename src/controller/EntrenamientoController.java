	...
	private void ejecutarAtaque(int index) {
		...
		if (pokemonSalvaje.estaDebilitado()) {
			...
			try (Connection con = ConexionBD.conectar()) {
				...
				int umbral = pokemonAliado.getNivel() * 50;
				...
				if (nuevaExp >= umbral) {
					...
					pokemonAliado.setVitalidad(pokemonAliado.getVitalidad() + 10);
					pokemonAliado.setAtaque(pokemonAliado.getAtaque() + 5);
					pokemonAliado.setDefensa(pokemonAliado.getDefensa() + 5);
					pokemonAliado.setVelocidad(pokemonAliado.getVelocidad() + 5);
				}

				PreparedStatement ps = con.prepareStatement(
						"""
								    UPDATE POKEMON SET NIVEL = ?, VIDA_ACTUAL = ?, VITALIDAD = ?, ATAQUE = ?, DEFENSA = ?, VELOCIDAD = ? WHERE ID_POKEMON = ?
								""");
				...
				ps.setInt(6, pokemonAliado.getVelocidad());
				ps.setInt(7, pokemonAliado.getId());
				ps.executeUpdate();
			} catch (SQLException e) {
			...
			actualizarBarrasVida();
			mensaje += "¡Aparece un nuevo " + pokemonSalvaje.getNombre() + " salvaje!\n";
			log(mensaje.trim());
			return;
		} else {
